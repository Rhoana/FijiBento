...
//        @Parameter( names = "--blendType", description = "The type of image blending to use (BLEND_WEIGHTED_SIMPLE, BLEND_LINEAR)", required = false, converter = BlendTypeConverter.class, arity = 1 )
	...
	public static void main( final String[] args )
	{
		...
		for ( int i = firstLayer; i <= lastLayer; i++ )
		{
			...
			final TileSpec[] tileSpecs = TileSpecUtils.readTileSpecFile( layerIdToTileSpecFilename.get( curLayer ) );
			...
			for ( int t = 0; t < origTilesRendered.length; t++ ) {
				origTilesRendered[ t ] = new SingleTileSpecRender( tileSpecs[ t ], params.numThreads );
			}
			
			// Pre-render all original tiles, so we can have a parallel version that saves
			// the new tiles
			System.out.println( "Rendering all tiles in the tilespec of layer " + curLayer );
			for ( int t = 0; t < origTilesRendered.length; t++ ) {
				origTilesRendered[ t ].render();
			}
			...
			saveAsTiles( 
					String.format( "%04d_%s", curLayer, params.outputNamePattern ),
					params.outputType,
					origTilesRendered,
					bbox,
					params.tileSize, 
			...
			String outFile = String.format( "%s/%04d_%s.png", params.targetDir, curLayer, baseFilename );
