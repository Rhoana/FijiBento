...
class RenderTiles3D(Job):
    def __init__(self, dependencies, tiles_fname, output_dir, layer, scale, from_x, from_y, to_x, to_y, jar_file, output_file, tile_size, output_type, output_pattern, blend_type=None, threads_num=1):
        Job.__init__(self)
        self.already_done = False
        self.tiles_fname = '"{0}"'.format(tiles_fname)
        self.output_dir = '-o "{0}"'.format(output_dir)
        ...
        if blend_type is not None:
            self.blend_type = '--blend_type {0}'.format(blend_type)
        self.dependencies = dependencies
        self.threads = threads_num
        self.threads_str = "-t {0}".format(threads_num)
        self.memory = 33000
    ...
    def command(self):
        return ['python -u',
                os.path.join(os.environ['RENDERER'], 'scripts', 'render_tiles_3d.py'),
                self.output_dir, self.jar_file, self.from_layer, self.to_layer, self.scale,
                self.from_x, self.from_y, self.to_x, self.to_y, self.threads_str, 
                self.tile_size, self.output_type, self.output_pattern, self.blend_type,
                self.tiles_fname]
...
#                        help='The output file name pattern where "%rowcol" will be replaced by "_tr[row]-tc[rol]_" with the row and column numbers')
    ...
    for f in json_files.keys():
        ...
        if not os.path.exists(render_out_file):
            print "Adding job for output layer: {0}".format(layer_output_dir)
            render_job = RenderTiles3D(bbox_and_norm_jobs, norm_list_file, layer_output_dir, layer, args.scale,
                args.from_x, args.from_y, args.to_x, args.to_y, args.jar_file, render_out_file, 
                args.tile_size, args.output_type, layer_output_pattern, args.blend_type, threads_num=8)
